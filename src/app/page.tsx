...
export default function LoginPage() {
  ...
  const handleSendEmail = async () => {
    ...
    setLoading('');
    if (authError) {
      setError('Email 發送失敗：' + authError.message);
      return;
    }
    setEmailSent(true);
  };
    ...
    <div className="min-h-dvh flex flex-col items-center justify-center px-6 relative overflow-hidden">
        ...
        <div className="w-full space-y-4 animate-slide-up relative z-10" style={{ animationDelay: '0.15s' }}>
          <div className="bg-white rounded-3xl p-6 space-y-3 shadow-sm" style={{ border: '1px solid #F2E8E0' }}>
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-sm font-medium text-text-secondary">選擇登入方式</p>
            </div>
            {/* Email — primary, always works */}
            <button
              className="btn-primary w-full flex items-center justify-center gap-2"
              onClick={() => { setStep('email'); setError(''); setEmailSent(false); }}
              disabled={!!loading}
            >
              <Mail size={18} /> Email 登入
            </button>
            {/* Google OAuth */}
            <button
      ...
      {step === 'email' && (
        <div className="w-full space-y-4 animate-slide-up relative z-10" style={{ animationDelay: '0.15s' }}>
          <div className="bg-white rounded-3xl p-6 space-y-4 shadow-sm" style={{ border: '1px solid #F2E8E0' }}>
            <div className="flex items-center gap-2">
              <button onClick={() => { setStep('methods'); setError(''); setEmailSent(false); }} className="p-1 rounded-full hover:bg-gray-100 transition-colors">
                <ArrowLeft size={20} className="text-text-secondary" />
              </button>
              <p className="text-sm font-medium text-text-secondary">Email 登入</p>
            </div>
            ...
            {!emailSent ? (
              ...
              <div className="text-center space-y-3 py-2">
                ...
                <div>
                  ...
                  <p className="text-xs mt-1" style={{ color: 'var(--text-secondary)' }}>請檢查 <strong>{email}</strong> 的信箱，點擊信中的連結即可登入</p>
                </div>
                <p className="text-[11px]" style={{ color: 'var(--text-secondary)', opacity: 0.6 }}>沒看到？請檢查垃圾郵件資料夾</p>
                <button
